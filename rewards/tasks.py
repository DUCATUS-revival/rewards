...
from rewards.models import AirdropStatus, Peer, Healthcheck, Airdrop, Reward
...
async def ping_nodes() -> None:
    ...
    logging.info(f'active nodes {active_enodes}')

    
    for enode in config.enodes:
        peer, _ = await Peer.get_or_create(enode=enode)
        await Healthcheck.create(peer=peer, online=enode in active_enodes)
...
async def check_waiting_airdrops():
    airdrops = await Airdrop.filter(
        status__in=(
            AirdropStatus.WAITING_FOR_RELAY, 
            AirdropStatus.INSUFFICIENT_BALANCE
        )
    ).select_for_update()
